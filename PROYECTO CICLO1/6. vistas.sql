...
-- Ver el nombre de los 5 productos mas prontos a vencer, y ver los dias que restan para que se dañe
SELECT id, nombre, fechaDeVencimiento, TRUNC(SYSDATE) - TRUNC(fechaDeVencimiento) AS diasRestantes FROM vencimiento_producto WHERE ROWNUM <= 5;
--Consultar los 10 proveedores con los precios mas bajos
SELECT * FROM precio_proveedor WHERE ROWNUM <= 10 ORDER BY compras ASC;
/*
--XIndices
